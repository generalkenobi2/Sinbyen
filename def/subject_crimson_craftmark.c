...
void configure() {
	::configure();
	set_subject_name("the craftmark of the Crimson Fist");
	set_subject_type(Subject_Type_Symbol);
	set_subject_description_known(
		"the craftmark of the Crimson Fist, an "
		"emblazoned image of a {{dark red}raised fist}"
	);
	set_subject_description_unknown(
		"an emblazoned image of a {{dark red}raised fist}"
	);
